# Floating Window Capsule Layout

## Purpose

Define how the voice input floating window should appear and resize during recording and transcription so it remains a single integrated capsule rather than a capsule layered over a larger panel.

This capability ensures the floating UI:
- Appears as one capsule from the first rendered frame
- Keeps waveform and text visually centered together
- Expands horizontally before switching to a taller two-line layout
- Truncates overflow inside the capsule without detached secondary UI
